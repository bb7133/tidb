...
func (p *PhysicalMergeJoin) initCompareFuncs() {
	...
	for i := range p.LeftJoinKeys {
		p.CompareFuncs = append(p.CompareFuncs, expression.GetCmpFunction(p.LeftJoinKeys[i], p.RightJoinKeys[i], p.LeftJoinKeys[i].RetType))
	}
}
...
func (p *LogicalJoin) constructIndexMergeJoin(
	...
	for _, plan := range indexJoins {
		...
		for i := range join.KeyOff2IdxOff {
			...
			compareFuncs = append(compareFuncs, expression.GetCmpFunction(join.OuterJoinKeys[i], join.InnerJoinKeys[i], join.InnerJoinKeys[i].RetType))
			outerCompareFuncs = append(outerCompareFuncs, expression.GetCmpFunction(join.OuterJoinKeys[i], join.OuterJoinKeys[i], join.OuterJoinKeys[i].RetType))
		}
